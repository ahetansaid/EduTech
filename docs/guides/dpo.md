# Guide Délégué à la protection des données

**Pour qui :** Le ou la DPO du ministère et son équipe.  
**Votre espace :** Conformité — vous y arrivez directement après la connexion (`/audit`).  
**Accès :** https://edu-tech-api-rho.vercel.app/audit · centre d'aide en ligne : https://edu-tech-api-rho.vercel.app/aide/dpo

> Vérifier que chaque accès aux données personnelles est justifié, repérer les usages anormaux et répondre à l'autorité de protection des données.
...
`/audit` — Qui a demandé quelle donnée, pour quelle finalité, avec quelle décision.

- Décisions journalisées, accès refusés, part de refus.
- Recherche, filtres par décision et par finalité, « Exporter (CSV) ».
- Refus par critère manquant, comptes les plus souvent refusés.
- « Tenter d'ouvrir un dossier » : preuve que les refus sont journalisés.

### Registre des traitements

`/audit/traitements` — Chaque usage des données personnelles.

- Finalité, base légale, destinataires, durée de conservation, criticité.

### État du service

`/plateforme/etat` — Santé et sécurité de la plateforme.

- Sécurité des 24 dernières heures : connexions, verrouillages, refus.

## Tâches pas à pas

### Contrôler les accès d'une personne

1. Dans le journal, cherchez son nom ou son identifiant.
2. Filtrez par « Refusées » pour voir les tentatives indues.
3. Touchez « Exporter (CSV) » pour conserver les lignes affichées.

### Démontrer la traçabilité

> Devant un auditeur, montrer un refus journalisé en direct vaut mieux qu'un document.

1. Dans « Vérifier la traçabilité », touchez « Tenter d'ouvrir un dossier ».
2. Le refus apparaît dans le journal, avec le critère manquant.

## Questions fréquentes

**Peut-on supprimer une ligne du journal ?**  
Non. Le journal est en ajout seul : personne ne peut modifier ni supprimer une entrée, pas même un administrateur.

**Que signifie « critère manquant » ?**  
Le critère qui a fait refuser l'accès : rôle, périmètre, relation (par exemple, pas d'enseignement dans la classe) ou finalité.
...
- Examinez chaque semaine les « Comptes les plus souvent refusés ».
- Conservez vos exports dans un espace protégé : ils contiennent des noms.
